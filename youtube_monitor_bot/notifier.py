...
def escape_markdown(text: str) -> str:
    """Escape special Markdown characters but not links."""
...
class TelegramNotifier:
    ...
    def _send_message(self, text: str, parse_mode: str = 'Markdown') -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    logger.info(f"Notification sent (fallback): {text[:50]}...")
                    return True
                return False
    ...
    def notify_new_video(self, video: Dict, channel_name: str):
        ...
        # Use Markdown link format - title and link together
        text = (
            f"{emoji} *Новое видео!*\n\n"
            f"📺 *{escape_markdown(channel_name)}*\n"
            f"🎬 [{escape_markdown(video['title'])}]({video_url})"
        )
